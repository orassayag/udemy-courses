...
import { CourseStatusEnum } from '../../core/enums/index.js';
...
class DomService {
  constructor() {
    ...
    this.courseLimitAccessDOM =
      '[data-purpose="safely-set-inner-html:limited-access-container:title"]';
  ...
  async createSingleCourses(data) {
    ...
    try {
      ...
      for (let i = 0; i < courses.length; i++) {
        ...
        const title = courseDataModel.getElementsByClassName(
          this.postDOMTitle
        )[0];
        const url = courseDataModel.getElementsByClassName(
          this.singleCourseURLDOM
        )[0];
        ...
        await globalUtils.sleep(
          countLimitService.countLimitDataModel
            .millisecondsTimeoutBetweenCoursesCreate
        );
      }
  ...
  async createCourseFullData(data) {
    ...
    try {
      ...
      const urls = dom.window.document.getElementsByClassName(
        this.udemyURLInputName
      );
      if (validationUtils.isExists(urls)) {
        const result = courseUtils.createCourseSingleData(
          urls[0].getAttribute(this.udemyURLAttribute)
        );
        ...
        await globalUtils.sleep(
          countLimitService.countLimitDataModel
            .millisecondsTimeoutBetweenCoursesUpdate
        );
      }
      ...
      courseService.coursesDataModel.coursesList[courseIndex] =
        await this.setCourseError(error, courseDataModel);
      if (this.validateErrorInARow(true)) {
        return true;
      }
    }
  ...
  async createCoursesListWithNames(data) {
    ...
    try {
      for (let i = 0; i < coursesDOMList.length; i++) {
        ...
        const url = innerCourse.getElementsByClassName(
          this.coursesListTitleDOM
        )[0];
  ...
  async createCoursesList(coursesLists) {
    ...
    for (let i = 0; i < coursesLists.length; i++) {
      ...
      try {
        const coursesDOMList = dom.window.document.getElementsByClassName(
          this.coursesListContainerDOM
        );
        if (validationUtils.isExists(coursesDOMList)) {
          if (
            await this.createCoursesListWithNames({
              courseDataModel: courseDataModel,
              courseIndex: courseIndex,
          ...
          if (
            await this.createCoursesListWithoutNames({
              courseDataModel: courseDataModel,
              courseIndex: courseIndex,
              courseURLsList: dom.window.document.getElementsByTagName(
                this.href
              ),
            })
          ) {
  ...
  validateErrorInARow(isError) {
    if (isError) {
      ...
      return (
        this.createUpdateErrorsInARowCount >=
        countLimitService.countLimitDataModel
          .maximumCreateUpdateErrorInARowCount
      );
    } else {
      this.createUpdateErrorsInARowCount = 0;
    }
  ...
  async setCourseError(error, courseDataModel) {
    return await courseService.updateCourseStatus({
      courseDataModel: courseDataModel,
      status: CourseStatusEnum.CREATE_UPDATE_ERROR,
      details: `Unexpected error occurred during the create update process. More details: ${systemUtils.getErrorDetails(
        error
      )}`,
    });
  }
  ...
  isValidateUdemyURL(udemyURL) {
    return (
      udemyURL &&
      udemyURL.indexOf(applicationService.applicationDataModel.udemyBaseURL) >
        -1
    );
  }
}
