...
import { CourseStatusEnum, StatusEnum } from '../../core/enums/index.js';
...
class CourseService {
  ...
  initiate(logCourse) {
    if (
      validationUtils.isExists(
        applicationService.applicationDataModel.keywordsFilterList
      )
    ) {
      for (
        let i = 0;
        i < applicationService.applicationDataModel.keywordsFilterList.length;
        i++
      ) {
        applicationService.applicationDataModel.keywordsFilterList[i] =
          textUtils.toLowerCaseTrim(
            applicationService.applicationDataModel.keywordsFilterList[i]
          );
      }
    }
  ...
  getUdemyCourseName(udemyURL) {
    ...
    udemyURL = udemyURL.replace(
      applicationService.applicationDataModel.udemyCourseURL,
      ''
    );
  ...
  async createCourse(courseDataModel) {
    ...
    courseDataModel.udemyURLCourseName = this.getUdemyCourseName(
      courseDataModel.udemyURL
    );
  ...
  updateCoursePrices(data) {
    ...
    if (status === CourseStatusEnum.PURCHASE) {
      this.coursesDataModel.totalPurchasePriceNumber +=
        originalPrices.priceNumber;
    }
    return courseDataModel;
  }
  ...
  async finalizeCreateUpdateCourses() {
      ...
      if (
        ...
        if (this.filter(courseDataModel)) {
          this.coursesDataModel.coursesList[i] = await this.updateCourseStatus({
            ...
            details:
              'The course has been filtered since no entered keywords have been found to match the course name.',
          });
        }
      }
    ...
    if (
      !validationUtils.isExists(
        this.coursesDataModel.coursesList.filter(
          (c) => c.status === CourseStatusEnum.CREATE
        )
      )
    ) {
  ...
  filter(courseDataModel) {
    ...
    const udemyKeywords = courseUtils.getUdemyURLKeywords(
      udemyURLCompare,
  ...
  async compareCourses(courseDataModel) {
    ...
    for (let i = 0; i < this.coursesDataModel.coursesList.length; i++) {
      ...
      if (
        courseDataModel.id === currentCourse.id ||
        currentCourse.status !== CourseStatusEnum.CREATE
      ) {
        continue;
      }
  ...
  createSessionCourses(urls) {
    for (let i = 0; i < urls.length; i++) {
      const url = urls[i];
      this.coursesDataModel.coursesList.push(
        new CourseDataModel({
