...
import { CrawlResultModel } from '../../core/models/index.js';
import {
  ColorEnum,
  CourseStatusEnum,
  ModeEnum,
...
class PuppeteerService {
  ...
  initiate() {
    ...
    this.timeout =
      countLimitService.countLimitDataModel.millisecondsTimeoutSourceRequestCount;
    this.pageOptions = {
  ...
  async createCourses() {
    ...
    const isSpecificPage =
      ...
      applicationService.applicationDataModel.specificCoursesPageNumber <
        countLimitService.countLimitDataModel.maximumPagesNumber;
    const { browser, page } = await this.initiateCrawl(true);
    try {
      // Go to the courses from single URLs.
      for (
        let i = 0;
        i < applicationService.applicationDataModel.pagesCount;
        i++
      ) {
        applicationService.applicationDataModel.status =
          StatusEnum.CREATE_COURSES;
        const pageNumber = isSpecificPage
          ? applicationService.applicationDataModel.specificCoursesPageNumber
          : i + 1;
        const url = `${applicationService.applicationDataModel.coursesBaseURL}/page/${pageNumber}/`;
        await page.goto(url, this.pageOptions);
        await page.waitForFunction(this.waitForFunction, {
          timeout: this.timeout,
        });
        ...
        courseService.coursesDataModel.totalCreateCoursesCount +=
          coursesResult.coursesCount;
        await globalUtils.sleep(
          countLimitService.countLimitDataModel
            .millisecondsTimeoutBetweenCoursesMainPages
        );
  ...
  async udemyLogin(data) {
    ...
    try {
      ...
      for (
        let i = 0;
        i <
        countLimitService.countLimitDataModel.maximumUdemyLoginAttemptsCount;
        i++
      ) {
        ...
        if (
          !(await page.$(domService.loginEmailDOM)) &&
          !(await page.$(domService.loginPasswordDOM))
        ) {
          await page.goto(
            applicationService.applicationDataModel.udemyLoginURL,
            this.pageOptions
          );
          await page.waitForFunction(this.waitForFunction, {
            timeout: this.timeout,
      ...
      await page.$eval(
        domService.loginPasswordDOM,
        (el, value) => (el.value = value),
        accountService.accountDataModel.password
      );
      await this.sleepAction();
      ...
      if (
        (await page.$(domService.loginErrorDOM)) ||
        (await page.$(domService.signInHeaderDOM))
      ) {
        await this.close(browser, true);
  ...
  async udemyPurchases(data) {
    for (
      let i = 1;
      i <= countLimitService.countLimitDataModel.maximumSessionsCount;
      i++
    ) {
      ...
      await globalUtils.sleep(
        countLimitService.countLimitDataModel
          .millisecondsTimeoutBetweenCoursesPurchase
      );
    }
    return data;
  }
  ...
  async udemyPurchasesSession(data) {
    ...
    for (
      let i = 0;
      i < courseService.coursesDataModel.coursesList.length;
      i++
    ) {
      ...
      if (!courseDataModel.udemyURL) {
        courseService.coursesDataModel.coursesList[i] =
      ...
      courseService.coursesDataModel.coursesList[i] =
        purchaseResult.courseDataModel;
      courseService.coursesDataModel.courseDataModel =
        purchaseResult.courseDataModel;
    ...
    try {
      ...
      if (await page.$(domService.courseNotExistsDOM)) {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.PAGE_NOT_FOUND,
          details: 'Course page does not found, no such course.',
          originalPrices: null,
        });
      }
      ...
      if (await page.$(domService.courseLimitAccessDOM)) {
        let status = null;
        let details = null;
        const courseH2 = await page.$eval(
          domService.courseLimitAccessDOM,
          (el) => el.textContent
        );
      ...
      if (await page.$(domService.coursesSuggestListDOM)) {
        const courseH1 = await page.$eval(
          domService.coursesSuggestListDOM,
          (el) => el.textContent
        );
        ...
        if (!courseBackground && courseH1.indexOf('Courses') > -1) {
          return await this.setCourseStatus({
            page: page,
            courseDataModel: courseDataModel,
            status: CourseStatusEnum.SUGGESTIONS_LIST,
            details:
              "Course page is not purchasable, it's a courses list suggestions page.",
            originalPrices: null,
          });
        }
      }
      ...
      if (await page.$(domService.courseIsPrivateDOM)) {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.PRIVATE,
          details: 'Course page is private. It has lock icon button.',
          originalPrices: null,
        });
      }
      ...
      if (enrollButton) {
        const enrollButtonText = await page.$eval(
        ...
        if (enrollButtonText.indexOf('Go to course') > -1) {
          return await this.setCourseStatus({
            page: page,
            courseDataModel: courseDataModel,
            status: CourseStatusEnum.ALREADY_PURCHASE,
            details:
              'The course already purchased in the past. No price label exists.',
            originalPrices: null,
          });
        }
      } else {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.ENROLL_NOT_EXISTS,
          details:
            "Can't purchase the course, because the enroll button not exists.",
          originalPrices: null,
        });
      }
      ...
      if (await page.$(domService.courseLanguageLabelDOM)) {
        const languageLabel = await page.$eval(
          domService.courseLanguageLabelDOM,
          (el) => el.textContent
        );
      ...
      const coursePriceLabel = await page.$eval(
        domService.coursePriceLabelDOM,
        (el) => el.textContent
      );
      if (!coursePriceLabel) {
        return await this.setCourseStatus({
      ...
      if (await page.$(domService.courseOriginalPriceDOM)) {
        const courseOriginalPriceLabel = await page.$eval(
          domService.courseOriginalPriceDOM,
          (el) => el.textContent
        );
        if (courseOriginalPriceLabel) {
          // Get the prices from the label and save them.
          originalPrices = courseUtils.getCoursePrices(
            courseOriginalPriceLabel
          );
        }
      }
      ...
      if (coursePriceLabel.indexOf('Free') === -1) {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.COURSE_PRICE_NOT_FREE,
          details:
            "The course price is not free. The keyword 'Free' doesn't exists in the price label.",
      ...
      if (await page.$(domService.purchaseSuccessDOM)) {
        ...
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.PURCHASE,
          details: 'Course has been purchased successfully.',
          originalPrices: originalPrices,
        });
      }
      ...
      if (!(await page.$(domService.checkoutPriceDOM))) {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.CHECKOUT_PRICE_NOT_EXISTS,
          details:
            "Course's price in the checkout page doesn't exists. No checkout price label exists.",
          originalPrices: originalPrices,
        });
      }
      this.logSessionStage('PAGE HAS CHECKOUT PRICE 1');
      // In the checkout page, Validate that the total price is 0.00.
      const checkoutPriceLabel = await page.$eval(
        domService.checkoutPriceDOM,
        (el) => el.textContent
      );
      ...
      if (priceNumber > 0) {
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.CHECKOUT_PRICE_NOT_FREE,
          details: `Course's priceNumber in checkout not equal to 0, but priceNumber equal to ${priceNumber}.`,
          originalPrices: originalPrices,
        });
      }
      ...
      if (await page.$(domService.purchaseSuccessDOM)) {
        ...
        return await this.setCourseStatus({
          page: page,
          courseDataModel: courseDataModel,
          status: CourseStatusEnum.FAIL,
          details:
            'The purchase has failed. Successfully purchase label does not exists.',
          originalPrices: originalPrices,
        });
      }
    } catch (error) {
      ...
      return await this.setCourseStatus({
        page: page,
        courseDataModel: courseDataModel,
        status: CourseStatusEnum.PURCHASE_ERROR,
        details: `Unexpected error occurred during the purchase process. More details: ${errorDetails}`,
        originalPrices: null,
      });
    }
  }
  ...
  async setCourseStatus(data) {
    ...
    const isErrorInARow = this.validateErrorInARow(
      status === CourseStatusEnum.PURCHASE_ERROR
    );
    return {
      page: page,
  ...
  checkPurchasedCount(status) {
    ...
    return (
      courseService.coursesDataModel.totalPurchasedCount >=
      countLimitService.countLimitDataModel.maximumCoursesPurchaseCount
    );
  }
  ...
  async udemyLogout(data) {
    ...
    try {
      ...
      await this.sleepAction();
      const urls = await page.$$eval(domService.href, (a) =>
        a.map((url) => url.href)
      );
      const urlIndex = urls.findIndex(
        (url) =>
          url.indexOf(applicationService.applicationDataModel.udemyLogoutURL) >
          -1
      );
      if (urlIndex === -1) {
        await this.close(browser, true);
  ...
  validateErrorInARow(isError) {
    if (isError) {
      ...
      return (
        this.purchaseErrorInARowCount >=
        countLimitService.countLimitDataModel.maximumPurchaseErrorInARowCount
      );
    } else {
      this.purchaseErrorInARowCount = 0;
    }
  ...
  async sleepAction() {
    await globalUtils.sleep(
      countLimitService.countLimitDataModel.millisecondsTimeoutUdemyActions
    );
  }

  async sleepLoad() {
    await globalUtils.sleep(
      countLimitService.countLimitDataModel.millisecondsTimeoutUdemyPageLoad
    );
  }
